  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-6xl mx-auto px-4">
              ...
              {appointments.map((appointment, index) => (
                <div key={appointment.id} className="bg-white p-4 rounded-lg border">
                  ...
                  <div className="mt-2 space-y-1">
                    ...
                    <div><strong>Duración:</strong> {appointment.duration || 'No definida'}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        {/* Test de fechas */}
        <div className="mt-8 bg-white p-4 rounded-lg border">
          <h3 className="text-lg font-semibold mb-2">📅 Test de Fechas</h3>
          <div className="space-y-2">
            {appointments.map(appointment => {
              const dateString = appointment.appointment_date
              const formattedDate = format(new Date(dateString), 'yyyy-MM-dd')
              const isMatch = dateString === formattedDate
              
              return (
                <div key={appointment.id} className={`p-2 rounded ${isMatch ? 'bg-green-100' : 'bg-red-100'}`}>
                  <div><strong>Fecha original:</strong> {dateString}</div>
                  <div><strong>Fecha formateada:</strong> {formattedDate}</div>
                  <div><strong>Coincide:</strong> {isMatch ? '✅' : '❌'}</div>
                </div>
              )
            })}
          </div>
